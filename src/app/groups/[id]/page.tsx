...
export default function GroupDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: inviteCode } = useInviteCode(groupId);
  const { data: groupStats } = useGroupStats(groupId);
  const { data: challengeIds } = useGroupChallenges(groupId);
          ...
          <div className="space-y-4">
            ...
            {challengeIds && challengeIds.length > 0 ? (
              ...
              <div className="card border-white/10 text-center py-12">
                ...
                <p className="text-sm text-stride-muted mb-4">Be the first to create a challenge for this group!</p>
                {isMember && (
                  <button
                    onClick={() => setShowCreateChallengeModal(true)}
